...
async def candidatura_approva(request: Request, user: dict = Depends(require_permission(100)), db=Depends(get_db)):
    from bson import ObjectId
    import nextcord
    from config import DISCORD_GUILD_ID, CANALE_ANNUNCI_ID, ROLE_MEDICO_TIROCINANTE_ID, ROLE_TIROCINIO_ID, ROLE_OSPEDALE_ID
    form = await request.form()
    cand_id = form.get("candidatura_id")
    ruolo_ids = [ROLE_MEDICO_TIROCINANTE_ID, ROLE_TIROCINIO_ID, ROLE_OSPEDALE_ID]
    ...
    try:
        ...
        if guild and ruolo_ids:
            try:
                ...
                member = None
            roles = [guild.get_role(int(r)) for r in ruolo_ids]
            ...
            if member and roles:
                ...
                embed.add_field(name="👔 Ruoli Assegnati", value=nomi_ruoli, inline=True)
            ...
            await db["dipendenti"].update_one(
                ...
                {"$set": {
                    "discord_id": discord_id,
                    "username": candidatura.get("username", ""),
                    "nome": candidatura.get("nome", ""),
                    "cognome": candidatura.get("cognome", ""),
                    "ruolo": "Medico Tirocinante",
                    "approvato": True,
                    "permission": 10,
